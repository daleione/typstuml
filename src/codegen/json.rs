...
//!
//! Walks the parsed `serde_json::Value` and emits a `#record-graph(...)` call
//! whose root is a recursive `(rows: ..., children: ...)` dict. Each JSON
//! object becomes one record (key/value rows); compound values produce
//! referenced child records connected by dashed arrows.
//!
...
//! etc. in JSON values render literally instead of being interpreted.
//!
...
/// Flatten one JSON value into the rows that will populate its record.
...
/// Escape characters that have special meaning in Typst markup so JSON
...
mod tests {
    ...
    fn object_array_spawns_one_child_per_element() {
        let out = render(json!({"phones": [{"n": 1}, {"n": 2}]}));
        // Two children share row 0 — array is transparent.
    ...
    fn empty_compound_emits_no_child() {
        let out = render(json!({"empty-arr": [], "empty-obj": {}}));
        // Placeholder rendered, but no `(row:` reference for either.
    ...
    fn scalars_get_special_markers() {
        let out = render(json!({"a": true, "b": false, "c": null}));
    ...
    fn title_is_emitted_in_record_graph_param() {
        ...
        emit(
            ...
            &JsonDiagram {
                name: None,
                title: Some("My JSON".into()),
                root: json!({"x": 1}),
            },
        );
        assert!(out.starts_with("#record-graph(title: [My JSON], "));
    }
}
